...
- **Accept — SPLIT into two clauses per D21 (2026-07-29, user).** The original single line bundled a **code property** with an **outreach obligation**; each is now stated on its own terms, so neither is satisfied by the other's progress. Same shape as the `source_status` two-meanings-in-one-field fix.
  - **(1) Code property — this clause gates the task and activates invariant 5:** grep shows no `requests.`/`urllib`/`httpx` usage outside `ingest/common.py`. *Wording caveat, unresolved and deliberately left in place: run literally, this trips on legitimate non-network `urllib.parse`/`urllib.error` helpers, so it is satisfiable only on a documented reading. Whether to retarget it at the actual fetch surface is escalated as **E19** and is NOT settled by D21 — until it is, any claim against this clause states which reading it passes on.*
