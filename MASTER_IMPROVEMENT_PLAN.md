...
Phases replace the v1.0 week schedule, which was unrealistic for a solo maintainer and contained a hard dependency inversion (the Phase-3 annotation study required an annotator recruited by a task previously scheduled after it). Phases are gated by exit criteria, not dates. Honest solo-pace expectation: Phase 1 alone is likely 4–8 weeks of elapsed time because WS0-T1 involves reading ~20 sets of legal terms and waiting on outreach replies. Work other Phase-1 tasks in parallel with that wait. Hardcoded minor-version release numbers below are illustrative, not commitments: the actual release number is determined at cut time from tag history, since patch/minor data releases can and do ship mid-phase (v2.7.0 and v2.8.0 both shipped during Phase-1 work; see D19, 2026-07-29). Phase 2–4 targets are major-version commitments tied to breaking changes and are not subject to that drift.

- **Phase 1 — "Honest" (→ next minor release at exit; v2.9.0 as of 2026-07-28).** Docs sync'd, licensing chain documented, overclaims removed, quick wins shipped, **second annotator recruitment started (WS5-T2a)**. *Exit criteria:* WS0-T2/T5/T6, WS6-T2, WS3-T5 (decision made) done; WS0-T1 has zero blank rows — every row resolved or carrying pending-outreach status with dates; WS5-T2a outreach sent; **WS0-T3 reduction complete for sources already confirmed non-redistributable (AIAAIC now; others as their WS0-T1 rows resolve); remaining sources gated on their WS0-T1 outcomes**; **WS0-T4 done** (D20(b)(i), 2026-07-29 — it activates invariant 5 and is conduct work this phase's "Honest" framing plainly covers; its absence from the original list was drafting, not design. Per **D21** this criterion is itself split: clause 1, the code property, gates the task; clause 2, outreach, tracks separately to completion); **WS4-T9 done — and met only when every dead source has EITHER a remediation implemented OR a hold carrying a DECISION DATE** (D20(b)(ii), 2026-07-29: an open-ended hold must read as *unsatisfied*. The AIRI hold qualifies only because its decision point is **2026-08-28**; **it did not qualify for 12 days**, and E18 exposed why — a hold whose clock cannot start is indistinguishable, on the old wording, from one merely waiting). *(Amended 2026-07-16: WS0-T3 was a P0 task with no phase assignment in v1.1 — the task that fixes what WS0-T1 documents was scheduled nowhere. The fold is deliberately scoped: an unscoped fold of the full ~20-source reduction would deadlock Phase 1 behind WS0-T1's outreach waits. Amended 2026-07-30 per D20(b): WS0-T4 added, WS4-T9's criterion sharpened. Both amend CRITERIA, not tasks, so the plan's task-count self-guard is untouched.)*
...
| 5 | All **HTTP(S)** fetching goes through `ingest/common.py` (rate limit, robots.txt, UA). **Non-HTTP egress** (vendor CLI such as `gh api`, `git clone`, any subprocess that reaches the network) is **not exempt by default** — it is out of this invariant's *mechanism* but inside its *accounting*: each instance is registered in `docs/INGESTION_CONDUCT.md` with its conduct properties, and **new non-HTTP egress requires its register entry in the same PR** (same convention as invariant 10's `SOURCE_LICENSES.md` rows). Amended by D22, 2026-07-29 — see the register for why "network" was too broad and "HTTP(S)" plus a register is sharper than either. | WS0-T4 done |
...
- **Accept — SPLIT into two clauses per D21 (2026-07-29, user).** The original single line bundled a **code property** with an **outreach obligation**; each is now stated on its own terms, so neither is satisfied by the other's progress. Same shape as the `source_status` two-meanings-in-one-field fix.
  - **(1) Code property — this clause gates the task and activates invariant 5.** *Retargeted at the actual fetch surface by D22 (2026-07-29, user), resolving E19.* **The fetch surface — `urlopen`, `requests.*`, `httpx.*` and the equivalents enumerated in the enforcer — appears nowhere outside `ingest/common.py`, with `scripts/scrape_aiid.py` as the single allowlisted inert exception** (permanently disabled; `Makefile` invocation commented out under D1/E1, guarded by a non-vacuity test). **Enforcer of record: `tests/test_network_chokepoint.py`**, which resolves call targets through import bindings and therefore catches aliased bypasses the original substring grep could not see, while shedding that grep's permanent false positives on legitimate non-network `urllib.parse`/`urllib.error` helpers. *Superseded wording, kept for the record: "grep shows no `requests.`/`urllib`/`httpx` usage outside `ingest/common.py`" — unsatisfiable as literally written (11 residual hits, zero of them live fetches), which is why it was retargeted rather than left to be flagged forever.*
  - **(2) Outreach obligation — tracks separately to completion; does NOT gate invariant 5:** outreach emails to AIID, AIAAIC and OECD AIM drafted, sent (human sends), and logged with dates in the doc. **Status 2026-07-29: AIAAIC sent `2026-07-27` (+ correction `2026-07-29`), AIID sent `2026-07-27`, MIT AIRI sent `2026-07-27` (courtesy) and `2026-07-29` (export request); OECD AIM = NO DRAFT EXISTS → drafted-awaiting-send.** Gated draft owed by license-auditor; the user sends and reports the date. This clause is **open** and must be reported as open.
