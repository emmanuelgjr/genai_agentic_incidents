...
## Composition
- **Instances:** consolidated incidents (<!-- stats:incident_count -->13,119<!-- /stats:incident_count -->
...
- **AIAAIC-derived rows carry an additional obligation.** 1,517 entries cite
  the AIAAIC Repository (CC BY-SA 4.0, share-alike) and carry a
  machine-readable `content_license` field recording a row-level
  attribution/share-alike obligation on that specific row — this does not
  apply to the dataset as a whole. A separate, open question about whether
  an EU/UK *sui generis* database right also applies to this extraction is
  pending AIAAIC's reply to outreach (sent 2026-07-27) or a
  qualified-counsel resolution; the current worst-case exposure is bounded
  to those same rows, never dataset-wide. Full framing: `NOTICE-DATA` and
