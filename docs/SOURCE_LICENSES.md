...
| Field | Value |
|---|---|
| License | **UNKNOWN.** `airi-navigator.com` (fetched 2026-07-15) and `/about` (fetched 2026-07-15) show no CC BY 4.0 or other license statement in rendered content. The `/about` page instead states the tool is *"a research prototype"* with data that is *"preliminary and not yet validated,"* and instructs users to *"verify against the original AIRI datasets before citing or relying on any specific numbers"* — a data-quality caveat, not a license grant. A web search suggested CC BY 4.0 for "MIT AI Risk Initiative" data generally, but this could not be confirmed by direct fetch and is not treated as established. **Re-checked 2026-07-16** after the AIAAIC miss showed WebFetch can lose content on large pages: re-fetched both pages with a targeted raw-substring search for "CC BY", "Creative Commons", "license", and "licence" (the same technique, applied deliberately this time rather than relying on a general summarizing prompt) — none present on either page. Unlike AIAAIC (a 4.4 MB page with a footer buried past a huge incident list), these are small, low-content pages, so the truncation mechanism that caused the AIAAIC miss doesn't plausibly apply here; the negative finding holds up under a second, more adversarial check. I don't have curl/shell access to go further than this. |
...
| 2 | AIAAIC Repository | **(b) share-alike, resolved by decision D2** — license confirmed CC BY-SA 4.0; handled by reducing to facts+link, not by split-licensing | Verbatim spreadsheet-cell text carried a real share-alike obligation; per D2 (2026-07-16, human) the project honors it by not carrying verbatim text at all, keeping one clean CC-BY-4.0 license with no BY-SA subset. Listed here as a resolved decision on record, not an open question. |
| 3 | OECD AIM | **(b)/(c) hybrid** | Narrative summary text is explicitly disclaimed by OECD as third-party IP. |
| 4 | MIT AIRI Navigator | **(d) UNKNOWN** | No license found; wraps AIID data (transitively CC-BY-SA). Re-checked 2026-07-16 with a targeted substring search (not just a summarizing fetch) after the AIAAIC miss raised doubt about negative findings generally — still nothing found; see §1.4 note. |
...
outreach-pending status; AIAAIC's licensing question is now resolved and
its handling decided (D2), so it's listed here as a recorded decision, not
an open question. Only **MIT AIRI Navigator and CSET-AIID Harm Taxonomy**
remain UNKNOWN with a dated pending-status record; OECD AIM carries a
narrower, still-genuine open question (does AIM's own summary text count as
OECD's reusable IP or third-party content?), also dated. Three rows total
carry the dated
