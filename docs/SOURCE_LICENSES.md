...
| Action | **(b)/(c) hybrid.** Reduce `description` to structural facts + link where the text is evidence-derived narrative; add "Source: OECD AI Incidents and Hazards Monitor" attribution per entry. Outreach still drafted, because the open question isn't OECD's general terms (now reasonably well corroborated above) but whether AIM's own summary/evidence text counts as OECD's IP under those general terms or as the third-party content AIM's own disclaimer flags — that specific boundary is what remains genuinely unresolved. **Implementation-status finding, added 2026-07-30, corrected 2026-07-30 (E21, red-reviewer/foreman verification):** the `description`-reduction this row calls for is **not yet implemented in code** — `scripts/ingest_oecd_aim.py::normalize_body()` still assigns `description = (summary or title).strip()`; that part of the finding stands. **Corrected characterization of what actually ships:** the `evidences` fallback branch fired for **zero of 4,160 rows checked** (length distribution min 253 / max 478 chars, none near the 1,000-char truncation ceiling; `summary` non-empty on 6/6 live pages sampled) — the shipping text is effectively 100% the `summary` path, with `evidences` present in code but currently inert, not the mixed source this row previously implied. **A separate, more consequential open question — escalated to the user, NOT resolved here:** on a deep-parsed page, `summary` does not appear anywhere inside the concatenated `evidences` blob; it reads as a composed OECD abstract, not quoted third-party news copy. If that holds generally, the third-party-disclaimer rationale in Redistribute-verbatim/Relicense-compatible above may not actually apply to the text this ingest ships, and the "OECD written content published from 1 July 2024 onward defaults to CC BY 4.0" fact in License (above) could instead govern it. **Do not act on this pending the user's resolution — in particular, do not rebuild or reduce any of the 3,667 AIM-derived descriptions currently in the corpus on the strength of this finding alone.** The code-level `description`-reduction fix remains correct to do regardless of how the licensing question resolves (structural-facts-only is strictly more conservative either way) — see the exact requirement in the report, routed to pipeline-engineer. |
...
| 3 | OECD AIM | **(b)/(c) hybrid — `description`-reduction to structural-facts-only not yet implemented in code; underlying licensing characterization now an OPEN QUESTION, escalated to the user (E21, 2026-07-30), NOT resolved here** | AIM's own methodology page disclaims third-party IP over its narrative content in general — established 2026-07-15, re-confirmed via raw HTML 2026-07-30. But red-reviewer/foreman verification (E21) found the text this ingest actually persists (`description`, sourced effectively 100% from the `summary` field — the `evidences` fallback fired on 0 of 4,160 rows checked) reads as **OECD-composed editorial** on the pages sampled, not quoted third-party news copy — which, if it holds generally, could put this content under §1.5's separate "OECD written content published from 1 July 2024 onward defaults to CC BY 4.0" finding instead of the third-party disclaimer. **Not adjudicated here — pending the user's resolution, and specifically not a basis to rebuild any of the 3,667 AIM-derived descriptions currently in the corpus.** The code-level fix (reduce `description` to structural facts + link) remains correct regardless of how this resolves, since it's strictly more conservative either way; see §1.5 Action. |
