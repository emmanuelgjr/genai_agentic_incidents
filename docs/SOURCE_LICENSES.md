...
| Field | Value |
|---|---|
| License | **CONFIRMED CC BY-SA 4.0.** aiaaic.org's site-wide footer (homepage and `/aiaaic-repository` — the exact page fronting the ingested sheet) states, verbatim: *"AIAAIC content is available to use, copy, adapt, and redistribute under a CC BY-SA 4.0 licence."* Only `/about-aiaaic` genuinely lacks the notice. **Verification caveat, stated plainly rather than hidden:** my fetch tool does not execute client-side JavaScript, and this footer sits in JS-rendered site chrome — repeated direct re-fetches of the homepage, `/aiaaic-repository`, `/projects`, `/contact`, and two leaf pages, including a targeted raw-substring search for "BY-SA," consistently came back empty for me (see report to the foreman for the full list of attempts). This confirmation rests on the reviewer's independently-run mechanical check (`grep -oic "CC BY-SA"` → 3 hits homepage, 1 hit `/aiaaic-repository`) rather than my own primary-source read. I was unable to independently reproduce it with my available tools, and I'm saying so rather than presenting it as self-verified. |
...
| Redistribute-verbatim | **YES, under share-alike terms.** CC BY-SA permits verbatim copying and adaptation, conditioned on redistributing under the same (or a compatible) share-alike license and giving attribution. `normalize_row()` currently copies AIAAIC's own cell text verbatim into our `description` field (headline, system, technology, purpose, ethical-issue labels, consequence, response) — this is now understood to be licensed, not a gap, provided the share-alike condition is honored downstream (see Action). |
| Relicense-compatible | **NO.** Share-alike (BY-SA) is definitionally incompatible with folding this content into a plain CC-BY relicense of the whole corpus — a derivative work built from CC BY-SA material must itself be CC BY-SA (or a license the CC BY-SA terms treat as compatible), not the project's blanket CC-BY-4.0. |
| Action | **(b) share-alike.** Per the plan: honor share-alike for this subset by splitting the license (AIAAIC-derived entries carry their own `license: CC-BY-SA-4.0` field/notice, distinct from the project's overall CC-BY-4.0 claim) rather than folding them into the blanket relicense. This is a direct input to WS0-T2 (LICENSE-DATA carve-out, same pattern as the MITRE ATLAS and garak Apache-2.0 carve-outs in §3.1/§3.3) and to WS0-T3 (a per-entry license-override field). A narrow, non-blocking clarification email to AIAAIC (attribution format preferred; confirming the recurring Google-Sheet CSV-export cadence is fine) is worth sending but does not gate anything — the licensing question itself is resolved, not pending. |
| Date-checked | 2026-07-15 (footer text originally per reviewer's mechanical check on the same date; my own re-fetch attempts also dated 2026-07-15) |
...
| 2 | AIAAIC Repository | **(b) share-alike, resolved** — license confirmed CC BY-SA 4.0; still needs a split-license/carve-out decision, not an outreach wait | Verbatim spreadsheet-cell text is licensed, but under share-alike terms incompatible with a blanket CC-BY relicense — needs the split-license decision recorded in WS0-T2/T3, not a pending-license wait. |
| 3 | OECD AIM | **(b)/(c) hybrid** | Narrative summary text is explicitly disclaimed by OECD as third-party IP. |
| 4 | MIT AIRI Navigator | **(d) UNKNOWN** | No license found; wraps AIID data (transitively CC-BY-SA). |
...
outreach-pending status; AIAAIC's core licensing question is now resolved
(§1.1) and is listed here only because the split-license decision itself
still needs recording, not because anything remains genuinely unknown. Only
**MIT AIRI Navigator and CSET-AIID Harm Taxonomy** remain UNKNOWN with a
dated pending-status record; OECD AIM carries a narrower, still-genuine open
question (does AIM's own summary text count as OECD's reusable IP or
third-party content?), also dated. Three rows total carry the dated
pending-status fields (MIT AIRI Navigator, OECD AIM, CSET-AIID). Outreach
